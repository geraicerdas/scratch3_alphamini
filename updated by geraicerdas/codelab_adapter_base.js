...
class AdapterBaseClient {
    ...
    constructor (
        ...
        this.socket = io(
            `${window.__static ? 'http:' : ''}//${adapterHost}:12358` +
                `/test?token=${token}`,
            {
        ...
        this.socket.on('sensor', msg => {
            ...
            case ADAPTER_TOPIC: {
                ...
                console.debug(`ADAPTER_TOPIC message->`, content);
                // 处理对应id的resolve
            ...
            case this.LINDA_CLIENT: {
                ...
                console.debug(`LINDA_CLIENT message->`, tuple);
                // 复用EIM位置参数
                if (typeof onAdapterPluginMessage === 'function') {
                    onAdapterPluginMessage(msg);
                }
        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (this._promiseResolves[messageID]) {
                    ...
                    this.runtime.emit('PUSH_NOTIFICATION', {
        ...
        if (this.runtime) {
            if (now - this.NOTIFICATION_lasttime > 1000) {
                ...
                try {
                    ...
                    this.runtime.emit('PUSH_NOTIFICATION', {
                        content: `rate limit (${this.SendRateMax})`,
                        type: 'error'
                    });
    ...
    send_to_linda_and_wait (node_id, payload, timeout = 5000) {
        // 返回promise
        // 返回 promise reject？
        if (!(Array.isArray(payload.tuple) && payload.tuple.length > 0)) {
            return Promise.reject('input error');
        }
        const messageID = this.send_to_linda(node_id, payload, timeout);
        return this.get_reply_message(messageID, timeout); // todo timeout
    }

    // 1000
    _linda_operate (operate, tuple, timeout = 1000 * 3600 * 24) {
        const node_id = 'linda/js/client';
        const payload = {};
